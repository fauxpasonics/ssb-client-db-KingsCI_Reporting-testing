...
[loaddate] [datetime] NULL CONSTRAINT [DF__Account__loaddat__1352D76D] DEFAULT (getdate()),
...
GO
ALTER TABLE [prodcopy].[Account] ADD CONSTRAINT [PK_Account] PRIMARY KEY CLUSTERED  ([Id])
GO
CREATE NONCLUSTERED INDEX [IX_PC_Account_isdeleted] ON [prodcopy].[Account] ([IsDeleted]) INCLUDE ([Archtics_ID__c], [Id])
GO
CREATE NONCLUSTERED INDEX [IX_PC_Acct_isdel] ON [prodcopy].[Account] ([IsDeleted]) INCLUDE ([Archtics_ID__c], [Id], [Name])
GO
CREATE NONCLUSTERED INDEX [IX_reno] ON [prodcopy].[Account] ([Reno_Bighorns_Account__c]) INCLUDE ([Id])
GO
